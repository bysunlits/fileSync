...
int sock_read()
{
	...
	printf("C TCP Server is listening on :%s:%.*s\n",(server_addr.sun_family==AF_UNIX)?"AF_UNIX":"UNKNOWN",(int)sizeof(server_addr.sun_path),server_addr.sun_path);
	...
	while (1)
	{
		...
		printf("New connection established,from Nginx:%s:%.*s\n",(client_addr.sun_family==AF_UNIX)?"AF_UNIX":"UNKNOWN",(int)sizeof(client_addr.sun_path),client_addr.sun_path);
